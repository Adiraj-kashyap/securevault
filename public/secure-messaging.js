...
document.addEventListener("DOMContentLoaded", () => {
    ...
    enterChatBtn.addEventListener("click", async () => {
        ...
        if (securityCode.length === 6) {
            try {
                const response = await fetch(`http://localhost:3000/chats/${securityCode}`);
                if (!response.ok) {
                    throw new Error(`HTTP error! status: ${response.status}`);
                }
                const data = await response.json();
    ...
    async function loadMessages() {
        try {
            const response = await fetch(`http://localhost:3000/messages/${currentChatId}`);
            if (!response.ok) {
                throw new Error(`HTTP error! status: ${response.status}`);
            }
    ...
    function renderMessages(messages) {
        ...
        messages.forEach((m) => {
            ...
            messageElement.className = `message ${m.sender === session.email ? "sent" : "received"}`;
